...
object AvroKafkaConsumer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    consumerProps.put("group.id", "person-consumer-group")
    ...
    consumer.subscribe(util.Arrays.asList("person-topic"))
    ...
    while(true) {
      ...
      records.forEach(record => {
        println("Person " + record.value.name + " status: " + record.value.status)
      })
